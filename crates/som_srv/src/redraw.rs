...
//! for the full history): `som-srv` needs to be a transparent PTY
//! proxy — Som's own `TerminalElement`/`TerminalView` must never change, so
//! whatever `som-srv` writes to its own stdout has to be plain ANSI
...
mod tests {
    ...
    /// up remnant of "...som-srv.exe" from the process list) and
