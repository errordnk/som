// `bounds`/`session`/`server`/`relay` implement somsrv's HOLDER/RELAY
...
fn parse_args() -> Args {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--version" => {
                println!("{}", serde_json::to_string(&somsrv::protocol::HandshakeInfo::current()).unwrap());
                std::process::exit(0);
            }
            // Self-contained admin subcommands — connect to the daemon
            // ALREADY running on this same machine (see `somsrv::admin`),
            // print a JSON result, exit. These are what `terminal_panel.
            // rs`'s `kill_orphaned_holders` replacement runs over SSH
            // (`ssh host somsrv --list-sessions ...`), mirroring the exact
            ...
            "--list-sessions" => {
                ...
                match somsrv::admin::list_sessions(client_id) {
                    Ok(sessions) => {
            ...
            "--kill-session" => {
                ...
                let Some(pane_id) = iter.next() else {
                    eprintln!("usage: somsrv --kill-session <client-id-or-empty-string> <pane-id>");
                    std::process::exit(2);
                };
                match somsrv::admin::kill_session(client_id, pane_id) {
    ...
    let usage = || -> ! {
        eprintln!(
            "usage:\n  somsrv [--client-id <id>] [--cursor-shape <shape>] [--scrollback <n>] <profile> <pane-id> <program> [args...]   (relay mode, what Som invokes)\n  somsrv --daemon   (shared daemon mode, spawned automatically — never invoked directly)"
        );
        std::process::exit(2);
    };
...
fn main() {
    ...
    let result = if args.daemon {
        ...
        match single_instance::try_acquire_daemon_lock() {
            ...
            Ok(None) => {
                log::info!("another somsrv daemon is already running, exiting");
                return;
            },
            Err(err) => Err(err),
        }
        ...
        // `ssh host ~/.local/bin/somsrv ...` spawned) — read here,
        ...
        let client_id = args.client_id.or_else(somsrv::protocol::ssh_client_id);
        relay::run(
            &args.profile,
            &args.pane_id,
            client_id,
...
fn init_logging(profile_name: &str, pane_id: &str, is_daemon: bool) {
    ...
    let log_path: &'static std::path::PathBuf = Box::leak(Box::new(if is_daemon {
        paths::logs_dir().join("somsrv-daemon.log")
    } else {
        paths::logs_dir().join(format!("somsrv-{profile_name}-{pane_id}-relay.log"))
    }));
