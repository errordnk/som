...
//! `somsrv` pane ids as a third, colon-separated segment:
...
//! that pane's `somsrv` pipe name (see `project_som_tmux` memory,
...
pub struct SomDbTab {
    ...
    /// One `somsrv` pane id per live pane (main first, then splits
