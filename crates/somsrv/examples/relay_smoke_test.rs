...
//! `cargo test` because it needs a REAL `somsrv.exe` sitting next
...
//! HOLDER). Run with:
//! `cargo run -p somsrv --example relay_smoke_test`
//!
//! Spawns `somsrv.exe` in RELAY mode with its stdio piped (the
...
fn main() {
    ...
    // adds; for THIS smoke test, forcibly kill any somsrv.exe
