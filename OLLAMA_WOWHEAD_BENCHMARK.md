...
### Решение по инфраструктуре (принято)

- Физического доступа к Mac mini НЕТ — только ollama по сети (`192.168.50.4:11434`).
- **Скрипт-воркер перевода крутится на Windows**, обращается к ollama на маке.
- Модель: **`gemma4:e4b-mlx`** (локальная, бесплатно).
- Windows должен быть включён весь прогон (~8–20 ч), но нагрузка на нём минимальна
  (сеть + запись чекпоинтов); вся работа на GPU мака. Запускать на ночь.
- Обязательны чекпоинты и возобновляемость — если Windows/сеть/мак отвалятся,
  продолжить с места остановки, не переводить заново.

### Ближайшие шаги на Windows

1. Дождаться скрапер, посмотреть выходной формат (JSON / CSV / SQLite), поля:
   questID, текст, автор, рейтинг/лайки, дата. Проверить качество: HTML/BB-разметка,
   ссылки на предметы, спойлеры, дубли, пустые.
2. Написать **скрипт батч-перевода** (Python, на Windows):
   - вход: файл скрапера (45k комментов)
...
4. Прогнать перевод (ночь), проверить чекпоинт на полноту, добить пропущенное.
5. Конвертер результата → **Lua data-файлы**: `questID -> { {автор, текст, рейтинг}, ... }`,
   разбить по N (лимиты WoW на размер файла и время загрузки аддона), подключить в `.toc`.
6. Написать **Lua/XML аддона**: загрузка data-таблиц, хук `QuestFrame`/`QuestLog`/
   `GameTooltip`, UI-блок комментариев (скролл, обрезка длины, «показать ещё»).

### Проверить перед прогоном

- Mac mini включён, не уснёт во время прогона: `sudo pmset -a sleep 0 disablesleep 1`
  (или через Настройки → Экономия энергии). Иначе через ~10 ч уснёт и оборвёт.
- Ollama отвечает по LAN: `curl http://192.168.50.4:11434/api/version`.
- Модель прогрета и не выгружается между батчами: слать с `"keep_alive": "30m"`
  в payload, иначе каждые 5 мин простоя = перезагрузка модели (~9 с).
- IP `192.168.50.4` не сменился (DHCP). Проверить или прописать `mac.local`.
