...
fn create_destination_list() -> anyhow::Result<(ICustomDestinationList, Vec<SmallVec<[PathBuf; 2]>>)>
{
    ...
    // Clear any previously registered tasks so stale entries (e.g. from old
    // builds that registered "New Window") are removed immediately.
    let _ = unsafe { list.DeleteList(None) };
