...
Пайплайн:
1. **Скрапер на Python уже качает данные** (на Windows): по **5 лучших комментариев**
   на квест, **~9 000 квестов** → **~45 000 комментариев** (≈10 МБ англ. текста).
   Формат/расположение выходного файла уточнить у пользователя в начале сессии.
...
### Оценка объёма перевода (45k комментариев)

| Метрика | Значение |
|---|---|
| Комментариев | ~45 000 (5 × ~9 000 квестов) |
| Текста EN | ~10 МБ / ~2.5 млн вх. токенов |
| Перевод RU | ~4 млн вых. токенов |

Время генерации на Mac mini M4 (`gemma4:e4b-mlx`, ~55 tok/s):

| Потоков | Время |
|---|---|
| 1 | ~20 ч + ~1 ч оверхеда (батч по 3) |
| 4 параллельно | ~5–6 ч |

Ollama на M4 держит несколько параллельных запросов, но при 4+ потоках делит
GPU и tok/s на поток падает — реальный выигрыш от параллелизма ~2.5–3×, не 4×.
Оптимально: **2–3 параллельных воркера, батч по 3 комментария**, чекпоинты на
диск (чтобы можно было прервать и продолжить). Итого реалистично **~8–10 часов**.

Альтернатива для чистового качества без вычитки — **Claude Haiku через API**
с кэшированным системным промптом: ~$15–20 за весь объём, ~1–2 часа с
параллелизмом.
...
### Решение по инфраструктуре (принято, обновлено 2026-09-06)

- Mac mini и Windows — один комплект через **KVM-переключатель** (одна клава/мышь/
  монитор, переключение между машинами). Физический доступ к маку ЕСТЬ.
- Claude работает на обеих машинах (одна и та же сессия/аккаунт).
- **Скрипт-воркер перевода крутится на самом Mac mini**, обращается к
  `http://localhost:11434` (НЕ по сети — нет LAN-обрывов и таймаутов).
- Модель: **`gemma4:e4b-mlx`** (локальная, бесплатно).
- Windows на время прогона не нужен вообще, может быть выключен.
- Sleep на маке уже предотвращён (`caffeinate, Claude, powerd`), пока сессия жива;
  в скрипт для надёжности обернуть `caffeinate -i`.
- Обязательны чекпоинты и возобновляемость — если процесс упадёт, продолжить
  с места остановки, не переводить заново.
- Скрапер (Python, качает 45k комментов) сейчас на Windows — **перенести на мак**
  (вставкой кода в чат / через git / переписать). Весь пайплайн будет на маке.
- Всё живёт в **отдельном git-репо для аддона** (не в репо Som).

### Ближайшие шаги (всё на Mac mini)

1. Создать отдельный git-репо для аддона (напр. `~/wow-quest-comments`).
2. Перенести скрапер с Windows, посмотреть выходной формат (JSON / CSV / SQLite),
   поля: questID, текст, автор, рейтинг/лайки, дата. Проверить качество:
   HTML/BB-разметка, ссылки на предметы, спойлеры, дубли, пустые.
3. Запустить скрапер на маке → дамп 45k комментов.
4. Написать **скрипт батч-перевода** (Python, на маке):
   - вход: дамп скрапера (45k комментов)
   - системный промпт + глоссарий WoW
   - параллельные воркеры (начать с 2–3, подобрать по факту — на M4 при 4+
     tok/s на поток падает), батч по 3 коммента, `gemma4:e4b-mlx`,
     эндпоинт `http://localhost:11434/api/chat`, `"keep_alive": "30m"`
   - **чекпоинт-файл** (jsonl: `{comment_id, quest_id, text_ru}` по мере готовности),
     при старте — пропускать уже переведённые ID
   - таймаут на запрос + ретраи
   - обёрнуто в `caffeinate -i`
   - постобработка по словарю замен (термины/имена, которые модель врёт)
   - на выходе собрать: `{questID: [{author, text_ru, rating}, ...]}`
   - лог скорости / ETA в консоль
5. Собрать **глоссарий терминов WoW** (Scarlet Crusade → Алый орден, wyrm → змей,
   DK → Рыцарь смерти, mob → моб, pull → пулить, aggro → агро, to tag → застолбить,
   phase → фаза, …) — и в промпт, и в словарь постобработки.
6. Прогнать перевод (~8–20 ч, на ночь), проверить чекпоинт на полноту, добить
   пропущенное.
7. Конвертер результата → **Lua data-файлы**: `questID -> { {автор, текст, рейтинг}, ... }`,
   разбить по N (лимиты WoW на размер файла и время загрузки аддона), подключить в `.toc`.
8. Написать **Lua/XML аддона**: загрузка data-таблиц, хук `QuestFrame`/`QuestLog`/
   `GameTooltip`, UI-блок комментариев (скролл, обрезка длины, «показать ещё»).

### Проверить перед прогоном

- Ollama отвечает: `curl http://localhost:11434/api/version`.
- Модель не выгружается между батчами: `"keep_alive": "30m"` в payload
  (иначе каждые 5 мин простоя = перезагрузка модели, ~9 с).
- Sleep предотвращён: `caffeinate -i python3 translate.py` (или `pmset`).
- Свободно на диске ~50 GB. Удалить лишние тестовые модели, чтобы освободить ~24 GB:
