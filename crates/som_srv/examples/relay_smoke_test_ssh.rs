...
//! `cargo test` for the same reason as that file: needs a real `som-srv.exe`
//! sitting next to whatever binary runs this. Run with:
//! `cargo run -p som_srv --example relay_smoke_test_ssh -- <ssh-host>`
//!
//! Reproduces EXACTLY what `terminal_panel.rs`'s `wrap_remote_command_args`
//! builds for a real `tmux: true` SSH profile: the LOCAL `som-srv.exe`'s
//! HOLDER spawns `ssh <host> ~/.local/bin/som-srv <profile> <pane-id>
//! $SHELL --cursor-shape block` as its real PTY child — that remote
//! `som-srv` then becomes ITS OWN local HOLDER/RELAY pair on the far side
...
//!
//! Requires: `~/.local/bin/som-srv` already deployed and runnable on the
...
fn main() {
    let host = std::env::args().nth(1).unwrap_or_else(|| {
        eprintln!("usage: cargo run -p som_srv --example relay_smoke_test_ssh -- <ssh-host>");
        std::process::exit(2);
    });
