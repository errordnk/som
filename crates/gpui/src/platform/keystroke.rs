...
pub struct Keystroke {
    ...
    /// Whether this key was pressed on a numeric keypad, when the platform
    /// can tell (currently macOS only; `false` elsewhere). A numpad "1" and
    /// a top-row "1" both produce the same `key`, so this is the only way
    /// to distinguish them.
    ///
    ...
    /// The physical key that was pressed, encoded as its USB HID Usage ID
    /// (Usage Page 0x07, Keyboard/Keypad) — the same numbering used by the
    /// USB HID Usage Tables spec, independent of OS and keyboard layout.
    /// `key`/`key_char` report what character the active layout produces;
    /// this reports *which key that was*, so e.g. a Russian layout's "й"
    /// can still be recognized as physically occupying the Q position.
    /// `None` when the platform can't determine it.
    ///
    /// Deliberately excluded from `PartialEq`/`Eq`/`Hash` for the same
    /// reason as `is_numpad`: purely informational.
    #[serde(default)]
...
impl PartialEq for Keystroke {
    fn eq(&self, other: &Self) -> bool {
        self.modifiers == other.modifiers && self.key == other.key && self.key_char == other.key_char
    }
}
...
/// Which physical instance of a modifier key (there are usually two, one on
/// each side of the keyboard) produced a `Modifiers` reading. Purely
/// informational — see the note on `Modifiers::side`.
#[derive(Copy, Clone, Debug, Eq, PartialEq, Hash, Serialize, Deserialize, JsonSchema)]
pub enum Side {
    /// The left-hand instance of the key.
    Left,
    /// The right-hand instance of the key.
    Right,
}
...
pub struct Modifiers {
    ...
    /// Which physical key (left or right) produced this modifier reading,
    /// when the platform can tell them apart. `None` when unknown or when
    /// multiple modifier keys are held with different sides at once.
    ///
    /// Deliberately excluded from `PartialEq`/`Eq`/`Hash` (see the manual
    /// impls below): keymap matching, keystroke equality, and every
...
/// The state of the num lock key at some point in time. Only meaningful on
/// Windows and Linux — macOS numeric keypads don't have a numlock toggle
/// (they have a non-toggling "Clear" key instead, and always send digits),
/// so this is always `on: false` there.
#[derive(Copy, Clone, Debug, Eq, PartialEq, Default, Serialize, Deserialize, Hash, JsonSchema)]
pub struct Numlock {
