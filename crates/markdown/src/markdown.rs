...
mod tests {
    ...
    fn test_context_menu_link_initial_state(cx: &mut TestAppContext) {
        ...
        let (_, cx) = cx.add_window_view(|_, _| TestWindow);
        let markdown =
            cx.new(|cx| Markdown::new("Hello [world](https://example.com)".into(), cx));
        cx.run_until_parked();
    ...
    fn test_capture_for_context_menu(cx: &mut TestAppContext) {
        ...
        ensure_theme_initialized(cx);
        let (_, cx) = cx.add_window_view(|_, _| TestWindow);
        let markdown = cx.new(|cx| Markdown::new("text".into(), cx));
        cx.run_until_parked();
