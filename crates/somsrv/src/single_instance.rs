//! Guards against two `somsrv --daemon` processes running at once.
//!
...
//! `somsrv --daemon`, and BOTH will succeed at binding the pipe name —
...
//! a burst of preview activity, traced to exactly two `somsrv.exe`
...
mod imp {
    ...
    pub fn try_acquire() -> anyhow::Result<Option<DaemonLockGuard>> {
        ...
        let name: Vec<u16> = "somsrv-daemon-singleton\0".encode_utf16().collect();
...
mod imp {
    ...
    /// scheme.
    pub fn try_acquire() -> anyhow::Result<Option<DaemonLockGuard>> {
        let path = format!("/tmp/somsrv-{}.lock", unsafe { libc::getuid() });
...
mod tests {
    ...
    /// `somsrv --daemon` processes coexist before this module existed
