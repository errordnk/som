...
/// `ssh host ~/.local/bin/som-srv ...` is an EXPLICIT remote command
...
/// this HOLDER process (`som-srv`, itself already `exec`'d as that
...
mod tests {
    ...
    /// remote command) — `ssh host ~/.local/bin/som-srv ...` IS an
    /// explicit command, so sshd never prints it no matter what
    /// `som-srv` does after the fact. `motd_wrapped_command` reproduces
