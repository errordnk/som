...
use som_srv::protocol::{ConnectionKind, HandshakeInfo, HolderOutput, RelayInput};
...
/// Runs the RELAY: connects to (or spawns) the shared daemon, registers
/// this pane's session with it, then blocks for the lifetime of the
/// connection, shuttling bytes both ways. Returns once the daemon
/// disconnects or the shell it holds exits — at that point there's
/// nothing left for this process to proxy, so `main` just lets it exit
/// (Som sees its PTY's child process end, same as any other shell
/// exiting).
pub fn run(
    profile_name: &str,
    pane_id: &str,
    ...
    // This RELAY only exists at all for a `tmux: true` profile — a `tmux:
    // false` SSH/local profile never runs `som-srv` as a shell wrapper in
    // the first place (see `wrap_remote_command_args`/`terminal_panel.rs`
    // on the Som side, which only substitutes this binary in when `tmux:
    // true`). So `Register::tmux` is always `true` here — the field exists
    // on the wire message for the DAEMON's own bookkeeping (deciding
    // whether to keep this session around after this RELAY disconnects),
    // not because this RELAY itself has a choice to make.
    let tmux = true;
    ...
    let connection = Arc::new(connect_or_spawn_daemon()?);
    ...
    // Register SECOND, right after the handshake — identifies this
    // session to the shared daemon (which serves every pane on the
    // machine from one process now, see `server.rs`'s module doc
    // comment) so it can find an existing session on reconnect or spawn a
    // brand new one on first connect. See `RelayInput::Register`'s own
    // doc comment for why every field except the lookup key
    // (`client_id`/`pane_id`) is ignored by the daemon on a reconnect.
    send(
        &connection,
        &writer,
        &RelayInput::Register {
    ...
    // Sends the current pane size right after handshake+register, before
...
/// BE the shared daemon (reusing the same binary with a `--daemon` flag
/// — see `main.rs`), then connect to IT as a normal RELAY and write this
/// connection's `ConnectionKind::Relay` tag (see that type's doc
/// comment) before anything else. This process itself never becomes the
/// daemon directly.
fn connect_or_spawn_daemon() -> anyhow::Result<PipeConnection> {
    // This RELAY process re-spawns ITSELF (`current_exe()`) in `--daemon`
    // mode if nothing's listening yet — see `som_srv::daemon`'s own doc
    ...
    // neither of which IS the `som-srv` binary, so they pass a
    // discovered/deployed path instead of `current_exe()`).
    let connection = som_srv::daemon::connect_or_spawn(&std::env::current_exe()?)?;
    ConnectionKind::Relay.write_to(&connection)?;
    Ok(connection)
}
