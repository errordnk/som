...
2. Написать **скрипт батч-перевода** (Python, на Windows, дёргает ollama на Mac mini):
   - вход: файл скрапера
   - системный промпт + глоссарий WoW
   - 2–3 параллельных воркера, батч по 3 коммента, `gemma4:e4b-mlx`,
     эндпоинт `http://192.168.50.4:11434/api/chat`
   - чекпоинты (jsonl с уже переведёнными ID), возобновляемость
   - постобработка по словарю замен (термины/имена, которые модель врёт)
   - выход: `{questID: [{author, text_ru, rating}, ...]}`
...
4. Конвертер результата → **Lua data-файлы**: `questID -> { {автор, текст, рейтинг}, ... }`,
   разбить по N (лимиты WoW на размер файла и время загрузки аддона), подключить в `.toc`.
5. Написать **Lua/XML аддона**: загрузка data-таблиц, хук `QuestFrame`/`QuestLog`/
