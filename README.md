...
Usually you have to pick: a GPU-accelerated terminal with inline graphics, *or* Windows support, *or* tmux (which you still have to install and configure yourself). Som gives you all of it out of the box: SSH sessions that survive disconnects, images rendered via the Kitty Graphics Protocol, GPU rendering, tabs in the title bar, and split panes — one binary on Windows, macOS, and Linux, no install, no dependencies.
...
- **Zero installs or setup.** Download one file and you already have session persistence and in-terminal graphics. No `apt install tmux` on every server, no `.tmux.conf`, no external daemon for image previews — `som-tmux` deploys itself to the remote host, graphics work by default.
- **3/3 platforms, no cut corners.** Terminals in this class are usually Linux/macOS tools first, with Windows either missing or an afterthought. Som was built for three platforms on one engine from the start — the Windows build is native and gets exactly the same feature set, Kitty graphics and living SSH sessions included.
...
- **One portable binary, ~32MB** — no installer. `som` runs from any directory — config, theme, `som-tmux`, and a patched ConPTY are embedded and extract themselves on first launch. A fraction of the size of the full GPUI stack it's cut from.
...
- **Images in the terminal** — Kitty Image Protocol: `yazi` and other clients show full-color previews (see below).

## som-tmux: sessions that don't die
...
## Kitty Graphics Protocol: real images in the terminal

Som understands the Kitty graphics protocol, so file managers and viewers show actual images in it instead of a mosaic of colored characters. No external daemon, no overlay window — the image is painted by the same GPU pipeline as the text, and behaves like part of the terminal's content.

- **Works with `yazi` and other clients** — image previews right inside a Som tab, Windows included. Som advertises protocol support the way existing clients already expect, so nothing needs to be configured.
...
- **PNG and raw pixels, any size** — both PNG and RGB/RGBA streams are supported, including chunked transfer of multi-megabyte images. Images can be placed both cursor-relative and via Unicode placeholders — both protocol modes.
- **Survives an SSH disconnect** — through `som-tmux`, images come back along with the rest of the session content: reconnect to the remote host and previews are right where they were, no manual redraw needed.
...
- **Linux** — amd64. X11 and Wayland via GPUI; `som-tmux` deploys to remote Linux servers too.
...
| `terminal` + `terminal_view`            | PTY terminal emulator, tabs, splits, Kitty Graphics Protocol, `som-tmux` integration |
