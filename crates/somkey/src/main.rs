...
//! keymap code.
//!
//! The drawn layout matches the platform somkey is running on (macOS,
...
impl SomKey {
    ...
    fn key_down(&mut self, event: &KeyDownEvent, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        // somkey is a standalone diagnostic tool with no other key
...
/// is present at the `Full100` profile, which is what `somkey` now
...
fn main() {
    application().run(|cx: &mut App| {
        ...
        // NumLock, or ScrollLock toggled on before somkey even opens.
        window
            .update(cx, |view, window, cx| {
