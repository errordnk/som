...
//! `kill_orphaned_holders` in `terminal_panel.rs`) can reuse the exact
//! SSH-probe-and-read-stdout pattern `ensure_remote_binary_deployed`'s
//! `--version` probe already established, rather than opening a raw pipe
//! connection over SSH from Som's own side for something this simple.
