...
//! `cargo test` for the same reason as that file: needs a real `somsrv.exe`
//! sitting next to whatever binary runs this. Run with:
//! `cargo run -p somsrv --example relay_smoke_test_ssh -- <ssh-host>`
//!
//! Reproduces EXACTLY what `terminal_panel.rs`'s `wrap_remote_command_args`
//! builds for a real `tmux: true` SSH profile: the LOCAL `somsrv.exe`'s
//! HOLDER spawns `ssh <host> ~/.local/bin/somsrv <profile> <pane-id>
//! $SHELL --cursor-shape block` as its real PTY child — that remote
//! `somsrv` then becomes ITS OWN local HOLDER/RELAY pair on the far side
...
//!
//! Requires: `~/.local/bin/somsrv` already deployed and runnable on the
...
fn main() {
    let host = std::env::args().nth(1).unwrap_or_else(|| {
        eprintln!("usage: cargo run -p somsrv --example relay_smoke_test_ssh -- <ssh-host>");
        std::process::exit(2);
    });

    let exe = std::env::current_exe().unwrap().parent().unwrap().join("somsrv.exe");
    if !exe.is_file() {
        eprintln!("somsrv.exe not found at {exe:?} — build it first with `cargo build -p somsrv --bin somsrv`");
        std::process::exit(1);
    }
    ...
    println!("spawning LOCAL relay: {exe:?} {profile} {pane_id} ssh {host} ~/.local/bin/somsrv {profile} <remote-pane-id> $SHELL --cursor-shape block");
    let mut child = Command::new(&exe)
        .arg(profile)
        .arg(&pane_id)
        .arg("ssh")
        .arg(&host)
        .arg("~/.local/bin/somsrv")
        .arg(profile)
        .arg(&pane_id)
        .arg("$SHELL")
        .arg("--cursor-shape")
