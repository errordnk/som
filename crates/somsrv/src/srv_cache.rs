...
//! own in-memory watermark tracker for a transfer in progress. `somsrv`
//! deliberately does NOT persist chunk bytes anywhere (no disk file, no
//! backing store beyond a small bounded trailing window, see `RECENT_
//! BYTES_WINDOW`) — it only tracks the gap-tolerant contiguous-length
//! bookkeeping and forwards each chunk's own bytes, unchanged, to every
//! `SubscribeProgress` subscriber as part of the SAME `SrvResponse::
//! Progress` push that already carries the watermark numbers (see that
//! variant's own `chunk_offset`/`chunk_data` fields). A subscriber
//! (Som's `SrvProgressState`) is the only place these bytes end up
//! materialized anywhere durable — its own bounded, forward-only
...
//! filled the disk. Image/GIF/markdown initially kept a disk-backed
//! fallback of their own (a real file `put_chunk` wrote, read back by
//! `crates/terminal`'s `RichContentCache::record_progress`) after video/
//! audio went memory-only, but that was removed too once those three
//! content types moved onto the exact same in-memory buffer video/audio
//! already used — every content type now takes the SAME code path here.
//! `somsrv` has no GPUI dependency and cannot call into `crates/terminal`
//! directly on Som's behalf (that crate pulls GPUI in), which is why this
//! bookkeeping is duplicated here rather than shared.
//!
...
use somsrv::protocol::{ContentMetadata, ContentType, SrvRequest, SrvResponse};
use std::collections::{HashMap, VecDeque};
use std::sync::{Arc, Mutex};

/// How many of the most-recently-forwarded bytes `CacheEntry::recent_
/// bytes` keeps around, for every content type. Small on purpose — this
/// is NOT a return to the old unbounded-disk-cache design (the whole
/// point of Part 0's redesign was escaping that), just enough of a
/// trailing window that a `RequestByteRange` answering a stuck reader is
/// very likely to land inside it. 8MB comfortably covers a
/// `GrowingFileStream`/`GrowingSrvStream` on-demand fetch's own
...
struct CacheEntry {
    ...
    /// recently forwarded bytes — the ONLY byte-level fallback this cache
    /// keeps, for every content type. Exists to answer a `RequestByteRange`
    ...
    /// (not a return to the old unbounded-disk-cache design) — see
    /// `RECENT_BYTES_WINDOW`'s own doc comment. `(first_byte_offset,
    /// bytes)` — `first_byte_offset` is the absolute file offset
    /// `bytes`'s own first byte corresponds to.
    recent_bytes: (u64, VecDeque<u8>),
}
...
impl SrvCache {
    ...
    /// Applies one `PutChunk`: advances the contiguous-length watermark,
    /// tracks a bounded trailing window of recently-forwarded bytes (see
    /// `CacheEntry::recent_bytes`'s own doc comment), and pushes
    /// `SrvResponse::Progress` (carrying `content_type`/`metadata` — see
    /// `PutChunk`'s own doc comment for why these travel on every chunk)
    /// to every subscriber for this key. No disk I/O for ANY content
    /// type — see this module's own doc comment for the incident that
    /// motivated removing it entirely. The push ALWAYS carries this
    /// chunk's own `data` (via `chunk_offset`/`chunk_data`) regardless of
    /// whether either watermark moved — a subscriber's in-memory buffer
    /// needs every chunk's bytes delivered, not just the ones that
    /// happen to advance `contiguous_len`/`tail_available_from`.
    pub fn put_chunk(
        &self,
        session_id: u32,
        file_id: u32,
        ...
        inner.entries.entry(key).or_insert_with(|| CacheEntry {
            contiguous_len: 0,
        ...
        entry.metadata = metadata.clone();
        {
            ...
            let (window_start, window) = &mut entry.recent_bytes;
    ...
    /// for this key) and forgetting them — see `UnsubscribeProgress`'s own
    /// doc comment for why this exists (the only way to unblock that
    /// connection's blocking `read_message()` call from the daemon side).
    /// Silently a no-op if nobody is subscribed, same tolerance every
    /// other best-effort message here has.
    pub fn unsubscribe(&self, session_id: u32, file_id: u32) {
        let mut inner = self.inner.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        let key = (session_id, file_id);
        if let Some(subscribers) = inner.subscribers.remove(&key) {
            for subscriber in &subscribers {
                let _ = subscriber(SrvResponse::Unsubscribed { session_id, file_id });
            }
        }
    }
    ...
    pub fn route_byte_range_request(&self, session_id: u32, file_id: u32, request: SrvRequest) {
        ...
        // in-memory window if the request falls inside it, exactly as if
        // a real sender had replied with an ordinary out-of-order
        // `PutChunk`.
    ...
    fn serve_from_recent_bytes(&self, inner: &mut Inner, session_id: u32, file_id: u32, offset: u64, len: u64) {
        ...
        let (window_start, window) = &entry.recent_bytes;
        ...
        self.push_range_reply(inner, session_id, file_id, offset, chunk_data);
    }

    fn push_range_reply(&self, inner: &Inner, session_id: u32, file_id: u32, offset: u64, chunk_data: Vec<u8>) {
        let key = (session_id, file_id);
        let Some(entry) = inner.entries.get(&key) else { return };
        let contiguous_len = entry.contiguous_len;
        let tail_available_from = entry.tail_available_from;
...
mod tests {
    ...
    fn route_byte_range_request_forwards_to_the_registered_sender() {
        ...
        cache.put_chunk(1, 2, 0, b"hello world", 11, ContentType::Gif, test_metadata());
    ...
    #[test]
    fn unsubscribe_pushes_unsubscribed_to_every_subscriber_and_forgets_them() {
        let cache = SrvCache::new();
        let observed = Arc::new(Mutex::new(Vec::new()));
        ...
        cache.unsubscribe(1, 2);

        let observed_snapshot = observed.lock().unwrap();
        assert_eq!(observed_snapshot.len(), 1);
        assert!(matches!(observed_snapshot[0], SrvResponse::Unsubscribed { session_id: 1, file_id: 2 }));
        drop(observed_snapshot);

        // The subscriber was forgotten — a later `notify_stop_playback`
        // (or any other push) for the same key must not reach it again.
        cache.notify_stop_playback(1, 2);
        assert_eq!(observed.lock().unwrap().len(), 1, "unsubscribe must remove the subscriber, not just notify it once");
    }

    #[test]
    fn unsubscribe_is_a_silent_no_op_with_no_subscriber() {
        let cache = SrvCache::new();
        // No `subscribe` call at all — must not panic or error.
        cache.unsubscribe(99, 99);
    }
    ...
    fn route_byte_range_request_serves_from_recent_bytes_once_the_sender_route_fails() {
        ...
        cache
            .put_chunk(1, 2, 0, b"hello world", 11, ContentType::Audio, ContentMetadata::Audio {
                sample_rate: 44100,
                channels: 2,
                bits_per_sample: 16,
                duration_ms: 1000,
    ...
    #[test]
    fn route_byte_range_request_serves_from_recent_bytes_for_image_gif_markdown_too() {
        // `recent_bytes` covers EVERY content type identically now (not
        // just Video/Audio) — a late `RequestByteRange` for image/GIF/
        // markdown must be answered from the same in-memory trailing
        // window, with no disk fallback anywhere in this cache anymore.
        let cache = SrvCache::new();
        cache.register_sender_route(1, 2, Arc::new(|_| anyhow::bail!("connection closed")));
        cache.put_chunk(1, 2, 0, b"hello world", 11, ContentType::Png, test_metadata());
        ...
        let observed = observed.lock().unwrap();
        assert_eq!(observed.len(), 1, "must be served from the in-memory window even though the sender route is dead");
        let SrvResponse::Progress { chunk_offset, chunk_data, .. } = &observed[0] else {
            panic!("expected a Progress push, got {:?}", observed[0]);
        };
        assert_eq!(*chunk_offset, 0);
    ...
    /// No disk I/O for ANY content type — confirmed directly by asserting
    /// each chunk's own bytes are forwarded, byte-for-byte, to a
    /// subscriber via the `Progress` push's `chunk_offset`/`chunk_data`
    /// fields, with no filesystem interaction anywhere in `put_chunk`
    /// (see this module's own doc comment for the 16GB-video incident
    /// that motivated removing disk I/O entirely, for every content type).
    #[test]
    fn put_chunk_forwards_each_chunks_bytes_to_subscribers_via_the_progress_push() {
        ...
        cache.put_chunk(1, 2, 0, b"hello", 10, ContentType::Gif, test_metadata());
        cache.put_chunk(1, 2, 5, b"world", 10, ContentType::Gif, test_metadata());
    ...
    fn put_chunk_tracks_tail_available_from_independently_of_contiguous_len() {
        ...
        // not move.
        cache.put_chunk(1, 2, 0, b"hello", 15, ContentType::Gif, test_metadata());
        ...
        cache.put_chunk(1, 2, 10, b"!!!!!", 15, ContentType::Gif, test_metadata());
        ...
        cache.put_chunk(1, 2, 5, b"world", 15, ContentType::Gif, test_metadata());
