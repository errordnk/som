...
//! for the full history): `somsrv` needs to be a transparent PTY
//! proxy — Som's own `TerminalElement`/`TerminalView` must never change, so
//! whatever `somsrv` writes to its own stdout has to be plain ANSI
...
mod tests {
    ...
    /// up remnant of "...somsrv.exe" from the process list) and
