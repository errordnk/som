...
# release) into ~/.config/som/tmux/{platform}/ on ALL THREE machines, so
# any of them can act as a Som client and scp the right binary to whatever
# SSH server it's talking to (see som_tmux::protocol::platform_binaries_dir
...
mkdir -p "$WIN_TMUX_DIR/windows-amd" "$WIN_TMUX_DIR/macos-arm" "$WIN_TMUX_DIR/linux-amd"
scp "$DEB:~/som/target/release/som-tmux" "$WIN_TMUX_DIR/linux-amd/som-tmux"
scp "$MAC:~/som/target/release/som-tmux" "$WIN_TMUX_DIR/macos-arm/som-tmux"
cp "$(dirname "$0")/../target/release/som-tmux.exe" "$WIN_TMUX_DIR/windows-amd/som-tmux.exe"

echo "==> distributing full set to $DEB and $MAC"
for host in "$DEB" "$MAC"; do
  ssh "$host" "mkdir -p ~/.config/som/tmux/windows-amd ~/.config/som/tmux/macos-arm ~/.config/som/tmux/linux-amd"
  scp "$WIN_TMUX_DIR/windows-amd/som-tmux.exe" "$host:~/.config/som/tmux/windows-amd/som-tmux.exe"
  scp "$WIN_TMUX_DIR/macos-arm/som-tmux" "$host:~/.config/som/tmux/macos-arm/som-tmux"
  scp "$WIN_TMUX_DIR/linux-amd/som-tmux" "$host:~/.config/som/tmux/linux-amd/som-tmux"
  ssh "$host" "chmod +x ~/.config/som/tmux/macos-arm/som-tmux ~/.config/som/tmux/linux-amd/som-tmux"
done
