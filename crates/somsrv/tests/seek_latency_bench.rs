//! Measures how long `somsrv` takes to make a `RequestByteRange`'s
...
//! real `somsrv.exe` daemon process, and takes real wall-clock minutes
...
use somsrv::protocol::{ConnectionKind, ContentMetadata, ContentType, HandshakeInfo, SrvRequest, SrvResponse, VideoCodec};
...
fn daemon_binary_path() -> std::path::PathBuf {
    ...
    path.push(if cfg!(windows) { "somsrv.exe" } else { "somsrv" });
    path
}

fn connect_and_handshake(daemon_binary: &std::path::Path) -> PipeConnection {
    let connection = somsrv::daemon::connect_or_spawn(daemon_binary).expect("connecting to somsrv daemon");
...
fn measure_seek_latency_at_increasing_distances_on_a_large_file() {
    ...
    assert!(daemon_binary.is_file(), "somsrv.exe not found at {}", daemon_binary.display());
