...
8. [som-srv: keeping remote sessions alive](#som-srv-keeping-remote-sessions-alive)
...
| `~/.config/som/srv/{windows-amd,macos-arm,linux-amd}/som-srv` | Pre-built `som-srv` binaries used to deploy to remote hosts for SSH sessions with `"tmux": true` — auto-extracted from Som's own bundled copies on first use, not something you need to provide. |
...
| `tmux` | bool | `false` | Opt-in to the `som-srv` backend for this profile — keeps the session alive across Som restarts. See [som-srv](#som-srv-keeping-remote-sessions-alive). |
...
## som-srv: keeping remote sessions alive

Setting `"tmux": true` on a tab profile routes that tab's terminal through a small companion process (`som-srv`) instead of a plain PTY:
...
- **Images survive SSH reconnects on `tmux: true` tabs.** Reattaching to a `som-srv` session that had images on screen redraws them along with the rest of the scrollback, without a manual refresh.
